...
class FlysystemAdapter implements FileSystemAdapterInterface {
     ...
     *
     * Flysystem does not expose direct permission checks, so writability
     * is inferred by attempting a safe temporary write operation.
     *
     * @param string $path Path to check.
    ...
    public function is_writable( string $path ): bool {
        try {
            ...
            if ( $this->is_dir( $path ) ) {
                $tmp = rtrim( $path, '/' ) . '/.smliser_write_test';
                $this->fs->write( $tmp, 'test' );
                $this->fs->delete( $tmp );
                return true;
            }

            // If path is a file, test writability via its parent directory.
            if ( $this->is_file( $path ) ) {
                $dir = dirname( $path );
                if ( $dir === '.' ) {
                    return false;
                }
                return $this->is_writable( $dir );
            }
        } catch ( \Throwable $e ) {
            return false;
        }
    ...
    public function readfile( string $path, int $start = 0, int $length = 0, int $chunk_size = 1048576 ): bool {
        try {
            ...
            while ( ! feof( $stream ) ) {
                $read_size = $remaining !== null
                    ? min( $chunk_size, $remaining )
                    : $chunk_size;

                $buffer = fread( $stream, $read_size );
