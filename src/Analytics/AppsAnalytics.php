 ...
 */
class AppsAnalytics extends DataStore {
     ...
     */
    public function log_download( AbstractHostedApp $app, string $ip, string $user_agent ) : bool {
     ...
     */
    public function get_total_downloads( AbstractHostedApp $app ) : int {
...
/**
     ...
     */
    public function get_downloads_per_day( AbstractHostedApp $app, int $days = 30 ) : array {
     ...
     * @return int
     */
    public function get_todays_downloads( AbstractHostedApp $app ) : int {
        $today  = gmdate( 'Y-m-d' );
     ...
     * @return int
     */
    public function get_yesterdays_downloads( AbstractHostedApp $app ) : int {
     ...
     */
    public function get_downloads_on( AbstractHostedApp $app, string $date ) : int {
     ...
     */
    public function get_average_daily_downloads( AbstractHostedApp $app, int $days = 30 ) : float {
     ...
     */
    public function get_peak_download_day( AbstractHostedApp $app, int $days = 365 ) : ?string {
        $daily_counts = self::get_downloads_per_day( $app, $days );
     ...
     */
    public function get_download_growth_percentage( AbstractHostedApp $app, int $days = 30 ) : float {        
     ...
     * @return bool Always true — dispatch is fire-and-forget.
     */
    public function log_client_access( AbstractHostedApp $app, string $event_type, string $ip, string $user_agent ) : bool {
        $this->job_queue->dispatch(
            JobDTO::make(
     ...
     */
    public function get_total_client_accesses( AbstractHostedApp $app ) : int {
     ...
     */
    public function get_client_access_per_day( AbstractHostedApp $app, int $days = 30 ) : array {
        // Calculate the absolute historical lookup date via TimestampValue utility
        $date = TimestampValue::now()->subtractDays( $days )->format( 'Y-m-d H:i:s' );
     ...
     */
    public function get_average_daily_client_accesses( AbstractHostedApp $app, int $days = 30 ) : float {
     ...
     */
    public function get_client_access_event_breakdown( AbstractHostedApp $app, int $days = 30 ) : array {
        // Calculate the absolute historical lookup date via TimestampValue utility
        $date = TimestampValue::now()->subtractDays( $days )->format( 'Y-m-d H:i:s' );
     ...
     */
    public function get_estimated_active_installations( AbstractHostedApp $app, int $days = 30 ) : int {
        // Calculate the absolute historical lookup date via TimestampValue utility
        $date = TimestampValue::now()->subtractDays( $days )->format( 'Y-m-d H:i:s' );
     ...
     */
    public function get_peak_client_access_day( AbstractHostedApp $app, int $days = 365 ) : ?string {
        $daily = self::get_client_access_per_day( $app, $days );
     ...
     */
    public function get_client_access_growth_percentage( AbstractHostedApp $app, int $days = 30 ) : float {        
        // Compute structural boundary windows via TimestampValue utility
        $current_boundary  = TimestampValue::now()->subtractDays( $days )->format( 'Y-m-d H:i:s' );
        $previous_boundary = TimestampValue::now()->subtractDays( $days * 2 )->format( 'Y-m-d H:i:s' );
