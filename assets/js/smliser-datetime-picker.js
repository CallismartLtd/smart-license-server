 ...
 *
 *  Option: boundaryEl — HTMLElement that acts as the clipping boundary for
 *  positioning (e.g. a modal container). When omitted the picker auto-detects
 *  the nearest scrollable/clipping ancestor. Pass null to force viewport-only.
 *
 ...
 *  picker.setBoundaryEl(el|null)    → set/clear the clipping boundary element
...
class CallismartDatePicker {
    ...
    this.#opts = Object.assign(
      {
        ...
        boundaryEl:        undefined, // HTMLElement | null | undefined
        //   undefined → auto-detect nearest scroll/clip ancestor
        //   null      → use viewport only
        //   HTMLElement → use that element's visible rect
      },
  ...
  /**
   * Walk up the DOM from `el` and return the first ancestor that clips its
   * overflow (overflow hidden/auto/scroll, or a <dialog>/<[role=dialog]>).
   * Returns null when no such ancestor exists before <body>.
   * @param {HTMLElement} el
  ...
  #findScrollParent(el) {
    ...
    while (node && node !== document.body) {
      // <dialog> and role="dialog" are natural modal boundaries
      if (
        node.tagName === 'DIALOG' ||
      ...
      const clips = /auto|scroll|hidden|clip/.test(overflow + overflowY + overflowX);
  ...
  /**
   * Return the effective clipping boundary as a viewport-relative DOMRect.
   *
   *  - opts.boundaryEl = HTMLElement  → that element's bounding rect
   *  - opts.boundaryEl = null         → full viewport (no boundary)
   *  - opts.boundaryEl = undefined    → auto-detect nearest scroll/clip ancestor
   *
  ...
  #getBoundaryRect() {
    ...
    const { boundaryEl } = this.#opts;

    // Explicit null → caller wants viewport-only
    if (boundaryEl === null) return vp;

    // Explicit element → use its visible rect intersected with the viewport
    if (boundaryEl instanceof HTMLElement) {
      ...
      return {
        ...
        width:  Math.min(r.right,  vp.right)  - Math.max(r.left, 0),
        height: Math.min(r.bottom, vp.bottom) - Math.max(r.top,  0),
      };
    }

    // undefined → auto-detect
    const ancestor = this.#findScrollParent(this.#wrapper);
    if (ancestor) {
   ...
   *
   * Priority order for vertical placement:
   *   1. Below the input, if it fits inside the boundary.
   *   2. Above the input, if more space is available there.
   *   3. Whichever side has more room, clamped so the popup stays
   *      within the boundary rect (never bleeds under a modal edge).
   *
   * Respects:
   *   - opts.fixedHeaderOffset  — top gap for fixed page headers
   *   - opts.boundaryEl / auto-detection — modal/scroll container clipping
   */
  #position() {
    ...
    // Effective top boundary: whichever is lower — fixed-header offset or
    // the boundary container's own top edge
    const boundaryTop    = Math.max(boundary.top, offset);
    const boundaryBottom = boundary.bottom;
    const boundaryLeft   = boundary.left;
    const boundaryRight  = boundary.right;

    // Available space on each side (relative to the viewport)
    const spaceBelow = boundaryBottom - wrapRect.bottom;
    const spaceAbove = wrapRect.top   - boundaryTop;

    // ── Vertical placement ─────────────────────────────────────────────────
    let top;

    if (spaceBelow >= popH) {
      // Fits cleanly below — preferred position
      top = wrapRect.bottom + window.scrollY + 6;
    } else if (spaceAbove >= popH) {
      // Doesn't fit below, but fits above — flip
      top = wrapRect.top + window.scrollY - popH - 6;
    } else if (spaceAbove > spaceBelow) {
      // Neither side fits fully; more room above — open upward, clamp to boundary top
      top = wrapRect.top + window.scrollY - popH - 6;
      const minTop = window.scrollY + boundaryTop;
      if (top < minTop) top = minTop;
    } else {
      // More room below (or equal) — open downward, clamp to boundary bottom
      top = wrapRect.bottom + window.scrollY + 6;
      const maxTop = window.scrollY + boundaryBottom - popH;
      if (top > maxTop) top = maxTop;
    }

    // ── Horizontal placement ───────────────────────────────────────────────
    let left = wrapRect.left + window.scrollX;

    // Clamp so popup doesn't overflow the right boundary
    const maxLeft = window.scrollX + boundaryRight - popW - 4;
    if (left > maxLeft) left = maxLeft;

    // Clamp so popup doesn't overflow the left boundary
    const minLeft = window.scrollX + boundaryLeft + 4;
    if (left < minLeft) left = minLeft;

    this.#popup.style.top      = `${top}px`;
    this.#popup.style.left     = `${left}px`;
    this.#popup.style.minWidth = `${Math.max(wrapRect.width, 280)}px`;
  }
  ...
  /**
   * Set or replace the clipping boundary element used for positioning.
   *
   *   picker.setBoundaryEl(document.querySelector('.my-modal'))
   *   picker.setBoundaryEl(null)      // revert to viewport-only
   *   picker.setBoundaryEl(undefined) // revert to auto-detection
   *
   * @param {HTMLElement|null|undefined} el
   * @returns {this}
   */
  setBoundaryEl(el) {
    this.#opts.boundaryEl = el;
    if (!this.#popup.hidden) this.#position();
    return this;
  }
